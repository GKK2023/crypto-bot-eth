...
# CORRECTION: Support de la variable d'environnement ETH_ENTRY_PRICE
...
# Prix d'achat manuel (optionnel - définissez via variable d'environnement ETH_ENTRY_PRICE)
MANUAL_ENTRY_PRICE = os.getenv('ETH_ENTRY_PRICE', '')
...
class SimpleBot:
    def __init__(self):
        if PAPER_MODE:
            ...
            # CORRIGE: Récupérer le prix d'achat
            if eth_balance >= MIN_POSITION_THRESHOLD:
                ...
                if MANUAL_ENTRY_PRICE:
                    try:
                        ...
                        print(f"[DEBUG] Prix d'achat depuis la variable d'environnement: ${entry_price:.4f}")
                    except:
                        print(f"[DEBUG] Erreur lors de la lecture de ETH_ENTRY_PRICE")
                
                # 2. Sinon, essayer l'historique des ordres
                if not entry_price:
                    entry_price = self.get_entry_price_from_orders()
                
                # 3. Sinon, essayer l'historique des trades
                if not entry_price:
                    entry_price = self.get_entry_price_from_trades()
                
                if entry_price:
                    self.position = {'side': 'long', 'entry': entry_price, 'amount': eth_balance}
                    print(f"Position existante detectee: {eth_balance} ETH @ prix d'achat: ${entry_price:.4f}")
                else:
                    print(f"ATTENTION: Impossible de trouver le prix d'achat automatiquement!")
                    print(f"Definissez la variable d'environnement ETH_ENTRY_PRICE sur Render")
                    self.position = None
            else:
                print(f"Dust ignore: {eth_balance} ETH - Pas de position")
                self.position = None

    def get_entry_price_from_orders(self):
        """Recupere le prix d'achat depuis l'historique des ordres (plus fiable)"""
        try:
    ...
    def get_entry_price_from_trades(self):
        """Recupere le prix d'achat moyen depuis l'historique des trades"""
        try:
            ...
            if usdt_to_use > 5:
                ...
                if amount_after_fee * price >= 7:
                    ...
                    if PAPER_MODE:
                        ...
                        print(f"ACHAT simule: {amount:.4f} ETH a ${price}")
                    else:
                        order = self.exchange.create_order(SYMBOL, 'market', 'buy', usdt_to_use)
                        print(f"ACHAT reel: {amount:.4f} ETH a ${price}")
                        self.position = {'side': 'long', 'entry': price, 'amount': amount}
        except Exception as e:
            ...
            if eth_balance >= MIN_POSITION_THRESHOLD:
                ...
                if amount * price >= 7:
                    if PAPER_MODE:
                        ...
                        print(f"VENTE simulee: {amount:.4f} ETH a ${price}")
                        self.position = None
                    else:
                        order = self.exchange.create_order(SYMBOL, 'market', 'sell', amount)
                        print(f"VENTE reelle: {amount:.4f} ETH a ${price}")
                        self.position = None
        except Exception as e:
            print(f"Erreur vente: {e}")

    def run(self):
        print(f"\n===== DEMARRAGE DU BOT GATE.IO ETH =====")
        ...
        print(f"Seuil position minimum: {MIN_POSITION_THRESHOLD} ETH (dust ignore si <)")
        print(f"========================================\n")
                ...
                if data is not None:
                    ...
                    if price is not None:
                        ...
                        if self.position is None:
                            ...
                            if eth_balance < MIN_POSITION_THRESHOLD:
                                print(f" -> Dust ignore: {eth_balance:.6f} ETH - Position reinitialisee")
                                self.position = None
                                if self.should_buy(data):
